...
func TestStmtStr(t *testing.T) {
	testcases := []struct {
		...
		{
			actual:      selectCountBallsStmtStr,
			expected:    "SELECT COUNT(*) FROM euro WHERE ball1=? OR ball2=? OR ball3=? OR ball4=? OR ball5=?",
			description: "selectCountBallsStr",
		},
		{
			actual:      selectCountStarsStmtStr,
			expected:    "SELECT COUNT(*) FROM euro WHERE ls1=? OR ls2=?",
			description: "selectCountLSStmtStr",
		},
	}
...
func Example_countStars() {
	...
	stmt, err = prepareCountStarsStmt(context.TODO(), db)
	if err != nil {
		fmt.Println(err)
	}

	count, err := countStars(context.TODO(), stmt, 3)
	if err != nil {
		fmt.Printf("Match bet error. %v", err)
	}
