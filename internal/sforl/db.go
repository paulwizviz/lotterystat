...
var (
	...
	selectCountLBStmtStr    = fmt.Sprintf(`SELECT COUNT(*) FROM %[1]s WHERE %[2]s=?`, tblName, luckyBall)
)
...
func prepareCountLBStmt(ctx context.Context, db *sql.DB) (*sql.Stmt, error) {
	stmt, err := db.PrepareContext(ctx, selectCountLBStmtStr)
	if err != nil {
		return nil, err
	}
	return stmt, nil
...
func countLuckBall(ctx context.Context, stmt *sql.Stmt, luckball uint8) (LuckyBallFreq, error) {
	row, err := stmt.QueryContext(ctx, luckball)
	if err != nil {
		return LuckyBallFreq{}, nil
	}
	...
	for row.Next() {
		err = row.Scan(&count)
		if err != nil {
			return LuckyBallFreq{}, err
		}
	}
	return LuckyBallFreq{
		LuckyBall: luckball,
		Count:     uint16(count),
	}, nil
}
